...
public class Main {
    ...
    private static int parse(final String arg) {
        try {
            int temp = Integer.parseInt(arg);
            /* exception thrown if argument is not an integer */
            if (temp <= 0) {
                throw new NumberFormatException();
                /* thrown if argument is not positive */
            }
            return temp;
    ...
    private static void automatic(final int[] sizes, final int repeat) {
